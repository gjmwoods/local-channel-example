...
public class LocalChannelConnector implements ChannelConnector {
	...
	public ChannelFuture connect(BoltServerAddress ignored, Bootstrap bootstrap) {
		...
		bootstrap.handler(new NettyChannelInitializer(null, SecurityPlanImpl.insecure(), 100, authTokenManager, clock, logging));
	...
	private void installChannelConnectedListeners(ChannelFuture channelConnected, ChannelPromise handshakeCompleted) {
		...
		channelConnected.addListener(future -> pipeline.addFirst(new ConnectTimeoutHandler(100)));

		// add listener that sends Bolt handshake bytes when channel is connected
		channelConnected.addListener(
				new ChannelConnectedListener(null, new ChannelPipelineBuilderImpl(), handshakeCompleted, logging));
	}
